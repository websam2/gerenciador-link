  ...
  useEffect(() => {
    ...
    fetch("/data/aniversariante.json")
      ...
      .then((data) => {
        ...
        const currentMonthBirthdays = data.aniversariante.filter((person) => {
    ...
    <div className="relative">
      {birthdays.length > 0 ? (
        <div>
          ...
          >
            <h1 className="font-bold text-text xl:text-xl mr-4 uppercase">
              Aniversariantes de {currentMonth}
            </h1>
            <FontAwesomeIcon
              icon={faChevronDown}
              className={`absolute text-bg right-0 transition-transform ${
